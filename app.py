...
st.set_page_config(page_title="우리 가족 자산 마스터 v15.0", layout="wide")

# CSS: 연한 하늘색 사이드바(검정 글씨) 및 고가독성 레이아웃
st.markdown("""
    <style>
...
st.title("👨‍👩‍👧‍👦 우리 가족 통합 자산 프로젝션 v15.0")
st.markdown("---")
...
with st.sidebar.expander("👤 부부 소득 및 상여금", expanded=True):
    ...
    with h_tab:
        ...
        h_ret_age = st.number_input("은퇴나이", value=55, key="h_r_in")
...
# (4) 자산 및 대출 정보
with st.sidebar.expander("📈 기본 자산 & 대출", expanded=False):
...
with st.sidebar.expander("🏠 부동산 갈아타기 계획", expanded=False):
    ...
    for i, tr in enumerate(st.session_state.re_trades):
        ...
        st.info(f"매각대금(세후): {est_sale/10000:.2f}억 / Gap: {gap/10000:.2f}억")
...
with m_tab:
    period = st.radio("🔍 조회 기간", ["5년", "10년", "20년", "30년", "전체"], horizontal=True, index=4, key="period_sel")
    sub = df_res.head({"5년":5, "10년":10, "20년":20, "30년":30, "전체":len(df_res)}[period])

    # 메인 차트 (호버 순서: 연도 > 총자산 > 순자산 > 부채)
    fig = go.Figure()
    ...
    # 지출 차트 (만단위 호버)
    f_exp = go.Figure(go.Bar(x=sub["연도"], y=sub["월_지출_만"], marker_color="#f43f5e", 
                             hovertemplate="<b>%{x}년</b><br>금액: %{y:,.0f}만<extra></extra>"))
    f_exp.update_layout(title=dict(text="💸 월 평균 지출(만)", font=dict(size=22)), height=340, margin=dict(l=10, r=10, t=60, b=10), template="plotly_white")
    c4.plotly_chart(f_exp, use_container_width=True)

with t_tab:
    st.header("⚖️ 상세 세무 분석 리포트")
    t_disp = sub[["연도", "남편나이", "아내나이", "월_세금_만", "소득세_만", "보유세_만", "양도세_만", "취득세_만"]].copy()
    t_disp.columns = ["연도", "남편", "아내", "월세금(만)", "소득세(연)", "보유/종부세(연)", "주택 양도세", "신규 취득세"]
...
with s_tab:
    ...
    fig_div = go.Figure()
    fig_div.add_trace(go.Scatter(x=df_div["연도"], y=df_div["월_배당금"], fill='tozeroy', name="월 배당(만)",
                                 hovertemplate="<b>%{x}년</b><br>배당금: %{y:,.0f}만<extra></extra>"))
    fig_div.update_layout(title="은퇴 후 월 배당 현금흐름 성장", height=500, template="plotly_white", font=dict(size=18))
    st.plotly_chart(fig_div, use_container_width=True)
    st.metric("은퇴 시점 금융자산", f"{ret_row['금융자산_억']:.2f}억", delta=f"첫해 월 배당 {df_div.iloc[0]['월_배당금']:,.0f}만")

with d_tab:
    st.subheader("📋 전체 시뮬레이션 상세 데이터")
    n_cols = ["순자산_억", "총자산_억", "부동산_억", "금융자산_억", "예금_억", "대출_억", "월_순현금_만", "월_지출_만"]
