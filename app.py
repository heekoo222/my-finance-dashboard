...
def run_simulation():
    ...
    for year in range(start_yr, 2096):
        ...
        # 4. 부채 상환 (동적으로 변하는 c_debt 기준)
        interest_a = c_debt * debt_r
        if c_debt > 0 and (year < start_yr + debt_term + 10): # 상환 기간 연장 여유분
            if debt_type == "원리금균등":
                repay_a = (c_debt * debt_r * (1+debt_r)**15) / ((1+debt_r)**15 - 1) if c_debt > 0 else 0
                principal_a = repay_a - interest_a
            else:
                principal_a = c_debt / 20; repay_a = principal_a + interest_a
        ...
        if net_flow_y >= 0:
            ...
            c_inv += net_flow_y # 적자 시 금융자산 우선 차감
...
with m_tab:
    ...
    def draw_mini(data, col, title, color):
        ...
        return f
    c1, c2 = st.columns(2); c1.plotly_chart(draw_mini(sub, "부동산_억", "🏠 부동산(억)", "#f97316"), use_container_width=True)
    c2.plotly_chart(draw_mini(sub, "금융자산_억", "📈 금융투자(억)", "#8b5cf6"), use_container_width=True)
    c3, c4 = st.columns(2); c3.plotly_chart(draw_mini(sub, "예금_억", "💰 예금(억)", "#0ea5e9"), use_container_width=True)
...
with s_tab:
    ...
    for y in range(f_ret_yr, 2096):
        ...
        div_data.append({"연도": y, "월_배당금": m_div}); c_inv_val *= (1 + inv_gr)
