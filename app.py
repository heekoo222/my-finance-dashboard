...
st.set_page_config(page_title="Family Wealth Master v14.0", layout="wide")
...
st.title("🏦 Family Financial Asset & Tax Master v14.0")
...
with m_tab:
    ...
    # 지출 차트만 예외적으로 '만' 단위 호버 적용
    fig_spend = go.Figure(go.Bar(x=sub["연도"], y=sub["월_지출_만"], marker_color="#f43f5e", 
                                 hovertemplate="<b>%{x}년</b><br>금액: %{y:,.0f}만<extra></extra>"))
    fig_spend.update_layout(title=dict(text="💸 월 평균 지출(만)", font=dict(size=22)), height=320, margin=dict(l=10, r=10, t=60, b=10), template="plotly_white")
    c4.plotly_chart(fig_spend, use_container_width=True)

with t_tab:
    st.header("⚖️ 연도별 상세 세무 리포트")
    tax_cols = ["연도", "남편나이", "아내나이", "월_세금_만", "소득세_만", "보유세_만", "양도세_만", "취득세_만"]
    t_disp = sub[tax_cols].copy()
    t_disp.columns = ["연도", "남편나이", "아내나이", "월세금(만)", "근로소득세(연)", "보유/종부세(연)", "주택 양도세", "신규 취득세"]
...
with s_tab:
    ...
    for y in range(f_ret_yr, 2096):
        ...
        c_inv_val *= (1 + inv_gr) # 주가 및 배당성장 복리 반영
    
    df_div = pd.DataFrame(div_data)
    fig_div = go.Figure()
    fig_div.add_trace(go.Scatter(x=df_div["연도"], y=df_div["월_배당금"], fill='tozeroy', name="예상 월 배당금(만)",
                                 hovertemplate="<b>%{x}년</b><br>월 배당: %{y:,.0f}만<extra></extra>"))
    fig_div.update_layout(title="연도별 배당 현금흐름 성장 추이", height=500, template="plotly_white", font=dict(size=18))
...
with d_tab:
    st.subheader("📋 전체 기간 재무 상세 데이터 (한글)")
