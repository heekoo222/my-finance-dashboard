...
st.markdown("""
    ...
    html, body, [class*="css"] { 
        ...
        font-size: 18px !important; /* 전체 기본 글씨 크기 확대 */
    }
...
with st.sidebar.expander("👤 부부 소득 & 은퇴/연금", expanded=False):
    ...
    with h_tab:
        h_sal = st.number_input("월급", value=830); h_inc = st.number_input("인상률(%)", value=3.0, step=0.1) / 100
        h_ret = st.number_input("은퇴나이", value=55); h_p_age = st.number_input("연금나이", value=65); h_p_amt = st.number_input("예상연금", value=150)
    with w_tab:
        w_sal = st.number_input("아내 월급", value=500); w_inc = st.number_input("인상률(%)", value=3.0, step=0.1) / 100
        w_ret = st.number_input("은퇴나이", value=55); w_p_age = st.number_input("연금나이", value=65); w_p_amt = st.number_input("예상연금", value=130)
...
with st.sidebar.expander("🔄 부동산 갈아타기 전략", expanded=False):
    ...
    for i, tr in enumerate(st.session_state.re_trades):
        ...
        tr['use_inv'] = c1.number_input(f"금융자산", value=tr['use_inv'], key=f"tr_i_{i}")
        tr['use_debt'] = c2.number_input(f"대출", value=tr['use_debt'], key=f"tr_d_{i}")
        tr['use_cash'] = c3.number_input(f"예금", value=tr['use_cash'], key=f"tr_c_{i}")
...
def run_sim():
    ...
    for year in range(start_year, end_year + 1):
        ...
        total_income_annual = inc_h + inc_w + pen
...
with t1:
    ...
    fig.add_trace(go.Bar(x=sub["year"], y=sub["net_asset"], name="순자산(억)", marker_color='#10b981', 
                         customdata=sub[["total_asset", "age", "m_net_flow", "event", "m_living", "m_loan", "m_tax", "savings_rate"]],
                         hovertemplate="<span style='font-size:20px'><b>%{x}년 (%{customdata[1]})</b></span><br>" + 
                                       "순자산: <span style='color:#10b981; font-size:18px'><b>%{y}억</b></span> | " + 
                                       "총자산: <b>%{customdata[0]}억</b><br>" + 
                                       "월 저축액: <span style='color:#3b82f6'><b>%{customdata[2]:,}만</b></span> (저축률 %{customdata[7]}%)<br><br>" +
                                       "<b>[월 지출 내역]</b><br>" +
                                       "• 생활비: %{customdata[4]:,}만<br>• 원리금: %{customdata[5]:,}만<br>• 세금: %{customdata[6]:,}만<br>" +
                                       "• 이벤트: <b>%{customdata[3]}</b><extra></extra>"))
    fig.add_trace(go.Bar(x=sub["year"], y=sub["debt_asset"], name="부채(억)", marker_color='#ef4444', 
                         customdata=sub[["total_asset", "age", "m_net_flow", "event"]],
    ...
    # 지출 상세를 누적 막대로 변경
    fig_spend = go.Figure()
    for col, name, color in [("m_living", "생활비", "#3498db"), ("m_loan", "원리금", "#e67e22"), ("m_tax", "세금", "#95a5a6"), ("m_kid", "양육비", "#f1c40f")]:
...
with t2:
    ...
    final_table = df.rename(columns={
        ...
        "m_living": "생활비(만)", "m_loan": "원리금(만)", "m_net_flow": "월순현금흐름(만)", "savings_rate": "저축률(%)", "event": "이벤트"
    })
