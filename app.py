...
# 1. 페이지 설정 및 디자인
st.set_page_config(page_title="Family Financial Roadmap", layout="wide")
st.markdown("""
    <style>
...
st.info("💡 모든 입력(왼쪽 레버)은 **'만원'** 단위이며, 모든 결과(오른쪽 차트/표)는 **'억원'** 단위로 자동 환산됩니다.")

# --- 2. 데이터 영속성 (세션 스테이트) ---
# 모든 금액 데이터는 내부적으로 '만원' 단위로 저장하되, 출력 시에만 '억'으로 변환합니다.
...
with st.sidebar.expander("👤 부부 급여 및 은퇴", expanded=True):
    ...
    with h_tab:
        h_sal = st.number_input("남편 월급(만원)", value=830)
...
with st.sidebar.expander("🔄 부동산 갈아타기 계획", expanded=False):
    ...
    for i, tr in enumerate(st.session_state.re_trades):
        ...
        tr['use_inv'] = c1.number_input(f"금융자산(만원) {i}", value=tr['use_inv'], key=f"tr_i_{i}")
        tr['use_debt'] = c2.number_input(f"추가대출(만원) {i}", value=tr['use_debt'], key=f"tr_d_{i}")
        tr['use_cash'] = c3.number_input(f"예금활용(만원) {i}", value=tr['use_cash'], key=f"tr_c_{i}")
...
with st.sidebar.expander("🚀 특별 이벤트 추가/수정", expanded=True):
    ...
    for i, ev in enumerate(st.session_state.events):
        ...
        ev['name'] = e_col1.text_input(f"이벤트명 {i}", ev['name'], key=f"ev_n_{i}")
        ev['year'] = e_col2.number_input(f"년도 {i}", 2025, 2095, ev['year'], key=f"ev_y_{i}")
        ev['cost'] = e_col3.number_input(f"비용(만원) {i}", 0, 500000, ev['cost'], key=f"ev_c_{i}")
...
with st.sidebar.expander("📈 자산/세금 기본 가정", expanded=False):
    ...
    inv_init = st.number_input("현재 투자(만원)", value=10000)
    inv_gr = st.number_input("투자 수익률(%)", value=7.0) / 100
    debt_init = st.number_input("현재 대출(만원)", value=60000)
...
# --- 4. 시뮬레이션 엔진 (모든 계산은 만원 단위 -> 결과만 억 단위) ---
def run_sim():
    results = []
    c_re, c_inv, c_cash, c_debt = re_init, inv_init, 500, debt_init # 예금 500만원 초기값 가정
    ...
    for year in range(2025, 2096):
        ...
        # (1) 수입 계산 (연간, 세후)
        inc_h = (c_h_sal * 13) if h_age <= h_ret else 0 # 상여 포함 약 13회분 가정
        inc_w = (c_w_sal * 13) if w_age <= w_ret else 0
        pension = 0
        if h_age >= h_p_age: pension += (h_p_amt * 12)
        if w_age >= w_p_age: pension += (w_p_amt * 12)
        # 중요: 이 변수가 데이터프레임의 '연수입'이 됩니다.
        yearly_income_total = inc_h + inc_w + pension
        ...
        for tr in st.session_state.re_trades:
            if year == tr['year']:
                ...
                c_cash -= tr['use_cash']
                c_re = tr['new_price']
                current_event_text.append(f"🏠 {tr['new_price']//10000}억 매수")
        ...
        for kid in st.session_state.kids:
            ...
            if 0 <= ka <= 7: k_total_annual += kid['costs'][0]*12
            elif 8 <= ka <= 13: k_total_annual += kid['costs'][1]*12
            elif 14 <= ka <= 16: k_total_annual += kid['costs'][2]*12
            elif 17 <= ka <= 19: k_total_annual += kid['costs'][3]*12
            elif 20 <= ka <= 23: k_total_annual += kid['costs'][4]*12
        ...
        # (5) 자산 업데이트
        # 연간 기본 생활비 4,800만원(월 400) 가정
        net_cashflow = yearly_income_total - (4800 + k_total_annual + tax + interest + ev_cost)
        c_re *= (1 + re_gr)
        ...
        results.append({
            ...
            "연수입": round(yearly_income_total / 10000, 2),
            "이벤트": " ".join(current_event_text)
        })
...
# --- 5. 시각화 UI (단위: 억원) ---
tab_main, tab_table = st.tabs(["📊 자산 성장 로드맵", "📋 연도별 상세 데이터 표"])
...
with tab_main:
    ...
    def make_bar_chart(data, col_name, title, color):
    ...
    # '연수입' 컬럼을 정확하게 호출
    c1.plotly_chart(make_bar_chart(sub_df, "부동산", "🏠 부동산", "#f97316"), use_container_width=True)
    c2.plotly_chart(make_bar_chart(sub_df, "금융자산", "📈 금융자산", "#8b5cf6"), use_container_width=True)
    c3.plotly_chart(make_bar_chart(sub_df, "부채", "📉 부채", "#f43f5e"), use_container_width=True)
    c4.plotly_chart(make_bar_chart(sub_df, "연수입", "💵 연수입", "#10b981"), use_container_width=True)

with tab_table:
    st.subheader("📋 전체 기간 재무 데이터 상세 (단위: 억원)")
    # 요청하신 모든 항목을 총망라한 표
