...
st.info("💡 **입력(왼쪽)**: 만원 단위 | **결과(차트/표)**: 자산은 '억원', 월 지출은 '만원' 단위입니다.")
...
# (2) 생활비 가정
with st.sidebar.expander("💸 연간 생활비 설정", expanded=False):
...
# (4) 부동산 갈아타기 (신규 로직 반영)
with st.sidebar.expander("🔄 부동산 갈아타기 (매각/매수)", expanded=True):
    # 기본 상승률 정보를 먼저 가져옴 (매각가 추정을 위해)
    re_init_val = st.sidebar.number_input("현재 부동산(만원)", value=140000, key="re_init_side")
    re_growth_rate = st.sidebar.number_input("부동산 상승률(%)", value=4.0, key="re_gr_side") / 100
    
    if st.button("➕ 갈아타기 시나리오 추가"):
    ...
    for i, tr in enumerate(st.session_state.re_trades):
        ...
        tr['year'] = st.number_input(f"갈아타는 연도", 2025, 2090, tr['year'], key=f"tr_y_{i}")
        
        # 매각가 추정 (보수적으로 95% 수준 수령 가정 - 양도세/수수료)
        est_sale_val = re_init_val * ((1 + re_growth_rate)**(tr['year'] - 2025)) * 0.95
        st.write(f"👉 예상 매각 대금(세후): **{est_sale_val/10000:.2f} 억원**")
        
        tr['new_price'] = st.number_input(f"새 아파트 매수가(만원)", 0, 1000000, tr['new_price'], key=f"tr_p_{i}")
        
        gap = tr['new_price'] - est_sale_val
        st.warning(f"⚠️ 조달 필요 금액(Gap): **{gap/10000:.2f} 억원**")
        
        c1, c2, c3 = st.columns(3)
        tr['use_inv'] = c1.number_input(f"금융자산 활용", value=tr['use_inv'], key=f"tr_i_{i}")
        tr['use_debt'] = c2.number_input(f"대출(추가)", value=tr['use_debt'], key=f"tr_d_{i}")
        tr['use_cash'] = c3.number_input(f"예금 활용", value=tr['use_cash'], key=f"tr_c_{i}")
        
        current_funding = tr['use_inv'] + tr['use_debt'] + tr['use_cash']
        if abs(current_funding - gap) > 100: # 100만원 이상 차이날 때
            st.error(f"조달 금액 합계({current_funding/10000:.2f}억)가 Gap과 일치하지 않습니다.")
...
def run_sim():
    ...
    for year in range(2025, 2096):
        ...
        # 2. 부동산 갈아타기 (신규 로직: 사용자가 입력한 자금 출처 반영)
...
with t1:
    ...
    fig.add_trace(go.Bar(x=sub["year"], y=sub["net"], name="순자산(억)", marker_color='#10b981', 
                         ...
                         hovertemplate="<b>%{x}년 (%{customdata[1]})</b><br>순자산: %{y}억<br>총자산: %{customdata[0]}억<br>월지출액: %{customdata[2]:,}만<br>이벤트: %{customdata[3]}<extra></extra>"))
