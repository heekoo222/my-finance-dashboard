...
# 1. 페이지 설정 및 프리미엄 디자인
st.set_page_config(page_title="Family Wealth Master v5.0", layout="wide")
...
# --- 3. 사이드바: 정밀 레버 설정 (고유 키 부여로 에러 방지) ---
st.sidebar.header("⚙️ 시뮬레이션 환경 설정")

with st.sidebar.expander("📅 기본 정보 & 시작연도", expanded=True):
    start_year = st.number_input("시뮬레이션 시작 연도", value=2026, min_value=2024, key="conf_start_yr")
    h_tab, w_tab = st.tabs(["남편(95)", "아내(94)"])
    with h_tab:
        h_sal = st.number_input("남편 월급(만)", value=830, key="conf_h_sal")
        h_inc = st.number_input("남편 인상률(%)", value=3.0, key="conf_h_inc") / 100
        h_ret = st.number_input("은퇴나이", value=55, key="conf_h_ret")
        h_p_amt = st.number_input("예상연금(월/만)", value=150, key="conf_h_pamt")
    with w_tab:
        w_sal = st.number_input("아내 월급(만)", value=500, key="conf_w_sal")
        w_inc = st.number_input("아내 인상률(%)", value=3.0, key="conf_w_inc") / 100
        w_ret = st.number_input("아내 은퇴나이", value=55, key="conf_w_ret")
        w_p_amt = st.number_input("예상연금(월/만)", value=130, key="conf_w_pamt")

with st.sidebar.expander("📈 금융투자 & 자산 배분", expanded=False):
    inv_init = st.number_input("현재 금융투자금(만)", value=10000, key="conf_inv_init")
    inv_gr = st.number_input("연간 기대 수익률(%)", value=7.0, key="conf_inv_gr") / 100
    st.markdown("---")
    st.write("💰 **잉여 자금 배분 비율**")
    inv_ratio = st.slider("금융자산 배분율(%)", 0, 100, 90, key="conf_inv_ratio")
    cash_ratio = 100 - inv_ratio

with st.sidebar.expander("💳 대출 상세 설정", expanded=False):
    debt_init = st.number_input("현재 대출 잔액(만)", value=60000, key="conf_debt_init")
    debt_r = st.number_input("대출 이자율(%)", value=3.9, key="conf_debt_r") / 100
    debt_term = st.number_input("상환 기간(년)", value=30, key="conf_debt_term")
    debt_type = st.selectbox("상환 방식", ["원리금균등", "원금균등"], key="conf_debt_type")

with st.sidebar.expander("🏠 부동산 & 지출 & 이벤트", expanded=False):
    re_init_val = st.number_input("현재 부동산(만)", value=140000, key="conf_re_val")
    re_gr_rate = st.number_input("부동산 상승률(%)", value=4.0, key="conf_re_gr") / 100
    living_monthly = st.number_input("월 생활비(만)", value=400, key="conf_living")
    living_gr = st.number_input("생활비 인상률(%)", value=2.0, key="conf_living_gr") / 100
    if st.button("➕ 부동산 갈아타기 추가", key="btn_add_re"):
        st.session_state.re_trades.append({"year": 2035, "new_price": 300000, "use_inv": 0, "use_debt": 0, "use_cash": 0})
    if st.button("➕ 특별 이벤트 추가", key="btn_add_ev"):
        st.session_state.events.append({"name": "이벤트", "year": 2030, "cost": 0})

# --- 4. 시뮬레이션 엔진 (정밀 세금 로직 적용) ---
def run_simulation():
    res = []
...
# --- 5. 결과 시각화 및 대시보드 ---
m_tab, tax_tab, d_tab = st.tabs(["📊 자산 로드맵", "⚖️ 상세 세금 분석", "📋 데이터 시트"])

with m_tab:
    p_choice = st.radio("시뮬레이션 조회 기간", ["10년", "20년", "30년", "전체"], horizontal=True, index=3, key="main_p_choice")
    ...
    fig.add_trace(go.Bar(x=sub["year"], y=sub["net_asset"], name="순자산(억)", marker_color='#10b981',
                         ...
                         hovertemplate="<span style='font-size:22px'><b>%{x}년 (%{customdata[0]})</b></span><br>" + 
                                       "순자산: <span style='color:#10b981; font-size:20px'><b>%{y:.2f}억</b></span> | " + 
...
with tax_tab:
    ...
    fig_tax.update_layout(height=500, template="plotly_white", title="연도별 세목별 지출(만원)", font=dict(size=16))
    st.plotly_chart(fig_tax, use_container_width=True)

    # 세금 상세 표 (에러 수정 포인트: 숫자 열만 포맷 지정)
...
with d_tab:
    ...
    # 에러 방지: 문자열 포함 열을 제외한 숫자 열만 소수점 둘째자리까지 표시
